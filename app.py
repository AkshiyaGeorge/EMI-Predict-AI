...
with tab5:
    ...
    if is_running_locally():
        try:
            ...
            st.info(
            "📊 MLflow tracking is active in local mode. You can view detailed model metrics and artifacts directly via the MLflow UI.\n\n"
            "🔗 [Launch MLflow UI](http://127.0.0.1:5000)")
        
        # 🌐 Cloud fallback message
        st.info(
            "⚠️ MLflow tracking is unavailable in cloud mode. For full access to model metrics and artifacts, please run this dashboard locally with an active MLflow server.\n\n"
